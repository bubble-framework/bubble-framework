...
const WELCOME_MSG = `${get('large_blue_circle')} WELCOME TO THE BUBBLE CLI! ${get('large_blue_circle')}\n`;
...
const GITHUB_PAT_MSG = `Please provide a valid github access token with 'repo' permission (https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/creating-a-personal-access-token) ${get('coin')}\nEnter token: `;
...
const INIT_FINISHED_MSG = `You're all set! Make sure your newly added \`.github/workflows\` folder is pushed to your Github repo's \`main\` branch. With any new pull requests, we'll wave our magic bubble wands and start blowin' some bubbles! ${get('magic_wand')}`;
...
const WAIT_TO_DESTROY_MSG = `We'll get started popping all active bubbles in your repo! Grab a bubble tea ${get('bubble_tea')} and chew on this bubble-related pun while you wait...\n`;
...
const FOLDER_ALREADY_DELETED = `Looks like your local Bubble-related workflow files were already deleted! We love an easy cleanup... ${get('broom')}`;

const WAIT_TO_TEARDOWN_MSG = `Let's get this bubble bath drained! ${get('bathtub')} Hope you haven't gotten tired of our jokes yet...\n`;
...
const TEARDOWN_DONE_MSG = `\nWoohoo! ${get('tada')} All remaining AWS resources provisioned for your bubbles have been removed, and any remaining traces of Bubble have been cleared out. It's like we were never here! ${get('ghost')} As a final step, feel free to remove the \`.github/workflows\` folder from the \`main\` branch of your remote Github repo. See you during your next \`bubble init\`! ${get('hugging_face')}`;
...
const commandsOutOfOrder = (command) => (
  `Oops! Couldn' finish executing \`bubble ${command}\`. Always make sure you're executing commands in the correct order: init, list/detail, destroy, teardown ${get('wink')}`
);

const duplicateBubbleInit = (repo) => (
  `Looks like you already have an AWS IAM user named ${repo}-bubble-user! Have you already run \`bubble init\` previously? Only one execution of init is needed per repo ${get('wink')}`
);
...
const randomJokeSetup = (type) => {
  ...
  const setup = jokeSetups[randomIdx];

  return setup;
};

const waitForJokeSetup = (setup) => `${setup}...\n`;

const waitForJokePunchline = (setup, type) => {
  const source = determineJokeSource(type);
  const randomJokePunchline = source[setup];

  return `...${randomJokePunchline} ${get('joy')}\n`;
};

const waitForDBJokeCrickets = () => {
  const cricketEmoji = get('cricket');

  return `${cricketEmoji}${cricketEmoji}${cricketEmoji}...\n`;
};

const instructTeardown = (repo) => {
  const clockEmoji = get('mantelpiece_clock');
  const sudsEmoji = get('soap');
